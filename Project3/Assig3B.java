...
public class Assig3B
{
    ...
    public boolean findPhrase(int r, int c, String words[], String word, int num, char [][] grid)
    {
        ...
        if (r >= grid.length || r < 0 || c >= grid[0].length || c < 0)
            return false;
        else if (!findWord(r, c, words[num], 0, grid))
            return false;
        else
        {
            ...
            {
                found = findPhrase(ex + 1, ey, words, words[num ], num + 1, grid);  // Right
                if (!found)
                    found = findPhrase(ex, ey + 1, words, words[num], num + 1, grid);  // Down
                if (!found)
    ...
    public boolean findWord(int r, int c, String word, int loc, char [][] grid)
    {
        ...
        else  	// current character matches
        {
            ...
            {		// Try all four directions if necessary.
                answer = findWord(r, c+1, word, loc+1, grid);  // Right
                if (!answer)
                    answer = findWord(r+1, c, word, loc+1, grid);  // Down
                if (!answer)
                    answer = findWord(r, c-1, word, loc+1, grid);  // Left
                if (!answer)
                    answer = findWord(r-1, c, word, loc+1, grid);  // Up
